...
class MaskedRegion:
    ...
    def predict_fragments(self):
        # @todo 01 calculate / estimate statistics
        self.parse_stats()
        
        # Finds peaks
        peaksStart = self.find_peaks(self.start_positions+[0])
        peaksStop = self.find_peaks(self.stop_positions+[0])
        
        # Correct / filter noisy peaks
        peaksStart = self.smooth_filter_peaks(peaksStart)
        peaksStop = self.smooth_filter_peaks(peaksStop)
        
        # @todo rename to taceback
        # Trace start and stop positions together and obtain actual peaks
        for fragment in self.assemble_fragments(peaksStart,peaksStop,self.start_avg_lengths,self.stop_avg_lengths):
            yield fragment
    ...
    def assemble_fragments(self,pstart,pstop,pexpectedStart,pexpectedStop,prime_5_ext = 3,prime_3_ext=5,genomic_offset_masked_region=0):
